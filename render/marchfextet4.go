...
func (dc *dcache3) processCubeTet4(c *cube, output chan<- []*Tet4) {
	if !dc.isEmpty(c) {
		if c.n == 1 {
			...
			output <- mcToTet4(corners, values, 0, -1)
		} else {
			// process the sub cubes
			n := c.n - 1
			s := 1 << n
			// TODO - turn these into throttled go-routines
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{0, 0, 0}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{s, 0, 0}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{s, s, 0}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{0, s, 0}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{0, 0, s}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{s, 0, s}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{s, s, s}), n}, output)
			dc.processCubeTet4(&cube{c.v.Add(v3i.Vec{0, s, s}), n}, output)
		}
	}
}
