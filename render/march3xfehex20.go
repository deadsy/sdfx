...
func (dc *dcache3) processCubeHex20(c *cube, output chan<- []*Hex20) {
	if !dc.isEmpty(c) {
		if c.n == 1 {
			...
			output <- mcToHex20(corners, values, 0, -1)
		} else {
			// process the sub cubes
			n := c.n - 1
			s := 1 << n
			// TODO - turn these into throttled go-routines
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{0, 0, 0}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{s, 0, 0}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{s, s, 0}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{0, s, 0}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{0, 0, s}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{s, 0, s}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{s, s, s}), n}, output)
			dc.processCubeHex20(&cube{c.v.Add(v3i.Vec{0, s, s}), n}, output)
		}
	}
}
